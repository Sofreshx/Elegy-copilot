...
await test('shared registry record uses SHA-based repoId matching getRepoStateKey()', () => {
  return withTempDir(async (root) => {
    ...
    process.env.ELEGY_HOME = elegyHome;
    try {
      const plugin = await WorktreePlugin({ project: { path: repoDir } });
      ...
      const recordDir = path.join(elegyHome, 'repo-state', expectedRepoId, 'worktrees');
...
await test('session.created writes a session record and links the current worktree', () => {
  return withTempDir(async (root) => {
    ...
    try {
      ...
      const expectedRepoId = computeRepoId(repoDir);
      const sessPath = path.join(elegyHome, 'repo-state', expectedRepoId, 'opencode-sessions', 'sess-1.json');
      ...
      const wtPath = path.join(elegyHome, 'repo-state', expectedRepoId, 'worktrees', worktreeId + '.json');
...
await test('session.idle marks session idle without releasing worktree assignment', () => {
  return withTempDir(async (root) => {
    ...
    try {
      ...
      const sessPath = path.join(elegyHome, 'repo-state', expectedRepoId, 'opencode-sessions', 'sess-2.json');
      const sess = JSON.parse(fs.readFileSync(sessPath, 'utf8'));
...
await test('session.error marks session error and worktree interrupted', () => {
  return withTempDir(async (root) => {
    ...
    try {
      ...
      const sessPath = path.join(elegyHome, 'repo-state', expectedRepoId, 'opencode-sessions', 'sess-3.json');
      const sess = JSON.parse(fs.readFileSync(sessPath, 'utf8'));
...
await test('session.deleted marks session deleted and worktree reusable', () => {
  return withTempDir(async (root) => {
    ...
    try {
      ...
      const sessPath = path.join(elegyHome, 'repo-state', expectedRepoId, 'opencode-sessions', 'sess-4.json');
      const sess = JSON.parse(fs.readFileSync(sessPath, 'utf8'));
...
await test('legacy session.create/session.delete are tolerated as running/deleted', () => {
  return withTempDir(async (root) => {
    ...
    try {
      ...
      const sessPath = path.join(elegyHome, 'repo-state', expectedRepoId, 'opencode-sessions', 'sess-legacy.json');
...
await test('worktree_create persists session record when OPENCODE_SESSION_ID is set', () => {
  return withTempDir(async (root) => {
    ...
    try {
      ...
      const expectedRepoId = computeRepoId(repoDir);
      const sessPath = path.join(elegyHome, 'repo-state', expectedRepoId, 'opencode-sessions', 'sess-create.json');
...
await test('session event resolves sessionID from properties.sessionID / sessionId / id / OPENCODE_SESSION_ID', () => {
  return withTempDir(async (root) => {
    ...
    try {
      ...
      const dir = path.join(elegyHome, 'repo-state', expectedRepoId, 'opencode-sessions');

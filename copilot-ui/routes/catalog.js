...
function buildManifestInventory(ctx) {
  ...
  const opencodeHomeAbs = ctx.opencodeHome || null;
  const codexHomeAbs = ctx.codexHome || null;
  const elegyHomeAbs = ctx.elegyHomeAbs || null;
      ...
      grouped[kind].push({
        ...
        actions: {
          ...
          installSurfaceTargets: INSTALL_SURFACE_HARNESSES.has(manifestSource.harnessId)
            ? [harnessIdToInstallSurfaceTarget(manifestSource.harnessId)]
            : [],
        },
        harnessStates: listHarnessRows(ctx)
          ...
          .map((harness) => buildHarnessState({
            ...
            assetId: assetId,
            isExternallyManaged: opencodeHomeAbs ? installLedgerLib.isAssetExternallyManaged(
              assetId, manifestSource.harnessId, opencodeHomeAbs, codexHomeAbs, elegyHomeAbs
            ) : false,
            detail: {
              readPath: sourceAbs || null,
              sourcePath,
...
function buildExternalSourceInventory(summary, ctx) {
  ...
  for (const source of sources) {
    ...
    for (const installable of installables) {
      ...
      if (kind !== 'mcp' && kind !== 'cli-tool' && kind !== 'plugin') {
        continue;
      }
  ...
  deps.readJsonBody(ctx.req)
    .then(async (body) => {
      ...
      if (optIn) {
        ...
        // Requires resolving destination paths from manifest asset definitions.
        // For v1, basic state model (available/not-installed/installed) is sufficient.
        ...
        installLedgerLib.removeHarnessOptIn(ctx.elegyHomeAbs, harnessId);
      }
      ...
      deps.sendJson(ctx.res, 200, {
        ...
        target,
        optedIn: optIn,
        assetCount: optIn ? collectManifestAssetIdsForHarness(deps.engineRoot || ctx.engineRoot, harnessId).length : 0,
      });
    })
    .catch((error) => sendJsonError(
      ctx.res,
...
async function handleHarnessAssetUninstall(ctx, deps) {
  deps.readJsonBody(ctx.req)
    .then(async (body) => {
      ...
      // Check secondary ledgers for externally managed assets
      const opencodeHomeAbs = ctx.opencodeHome || null;
      const codexHomeAbs = ctx.codexHome || null;
      const isExternalManaged = !isManaged && (opencodeHomeAbs || codexHomeAbs)
        ? installLedgerLib.isAssetExternallyManaged(assetId, harnessId, opencodeHomeAbs, codexHomeAbs, ctx.elegyHomeAbs)
        : false;
      ...
      if (!isManaged) {
        if (isExternalManaged) {
          deps.sendJson(ctx.res, 200, {
            ...
            deterministic: true,
            ok: false,
            warnings: [`Asset ${assetId} is externally managed for harness ${harnessId}. It cannot be deactivated from here — manage it through its source (e.g., Elegy skills).`],
          });
        } else {
          deps.sendJson(ctx.res, 200, {
            kind: 'catalog.harness_asset_uninstall',
            deterministic: true,
            ok: false,
            warnings: [`Asset ${assetId} is not managed by Elegy for harness ${harnessId}. Cannot safely deactivate unmanaged files.`],
          });
        }
        return;
      }
      ...
      try {
        ...
        if (!destPath) {
          ...
          installLedgerLib.setHarnessOptedIn(ctx.elegyHomeAbs, harnessId, updatedIds);
          deps.sendJson(ctx.res, 200, {
            kind: 'catalog.harness_asset_uninstall',
            deterministic: true,
            ok: true,
            removed: [assetId],
            warnings: ['Asset was not found on disk; removed from managed inventory only.'],
          });
          return;
        }
        ...
        if (ledgerHash && fileHash !== ledgerHash) {
        ...
        deps.sendJson(ctx.res, 200, {
          ...
          ok: true,
          removed: [assetId],
          warnings: hashWarning ? [hashWarning] : undefined,
        });
      } catch (error) {
        throw Object.assign(new Error(`Failed to uninstall: ${error.message}`), { statusCode: 500 });
      }
    })
    .catch((error) => sendJsonError(
      ctx.res,
      deps.sendJson,
      error.statusCode || 500,
      'catalog.harness_asset_uninstall',
      String(error.message || error),
    ));
}

async function handleHarnessAssetCheck(ctx, deps) {
  deps.readJsonBody(ctx.req)
    .then(async (body) => {
      const harnessId = normalizeString(body?.harnessId) || null;
      const assetId = normalizeString(body?.assetId) || null;
      const ledger = installLedgerLib.readInstallLedger(ctx.elegyHomeAbs);
      const results = [];

      // Determine which harnesses to check
      const harnessIds = harnessId
        ? [harnessId]
        : ['codex', 'opencode', 'antigravity', 'claude-code'];

      for (const hid of harnessIds) {
        const manifestFileName = hid === 'claude-code' ? 'claude-assets/manifest.json'
          : hid === 'codex' ? 'codex-assets/manifest.json'
          : hid === 'opencode' ? 'opencode-assets/manifest.json'
          : hid === 'antigravity' ? 'antigravity-assets/manifest.json'
          : null;
        ...
        try {
          ...
          for (const asset of assets) {
            ...
            result.installPath = destPath || null;

            // Check secondary ledgers for external-managed classification
            const opencodeHomeAbs = ctx.opencodeHome || null;
            const codexHomeAbs = ctx.codexHome || null;
            const isExternalManaged = opencodeHomeAbs || codexHomeAbs
              ? installLedgerLib.isAssetExternallyManaged(aid, hid, opencodeHomeAbs, codexHomeAbs, ctx.elegyHomeAbs)
              : false;
          ...
          results.push({ assetId: 'manifest-error', harnessId: hid, state: 'unknown', warnings: [`Failed to load manifest for ${hid}`] });
        }
      }
      ...
      deps.sendJson(ctx.res, 200, {
        ...
        summaryWarnings: results
          .filter(r => r.warnings && r.warnings.length > 0)
          .flatMap(r => (r.warnings || []).map(w => `${r.harnessId}/${r.assetId}: ${w}`)),
      });
    })
    .catch((error) => sendJsonError(
      ctx.res,
...
async function handleHarnessSync(ctx, deps) {
  deps.readJsonBody(ctx.req)
    .then(async (body) => {
      ...
      await deps.installSurfaces(installOptions);
      rebuildProjection(ctx, deps, {}, 'harness_sync');
...
function register(deps = {}) {
  ...
  return [
    ...
    {
      method: 'POST',
      path: '/api/catalog/harness-assets/uninstall',
      handler: (ctx) => handleHarnessAssetUninstall(ctx, resolvedDeps),
    },
    {
      method: 'POST',
    ...
    {
      method: 'POST',
      path: '/api/catalog/harness-assets/sync',
      handler: (ctx) => handleHarnessSync(ctx, resolvedDeps),
    },
    {
      method: 'POST',

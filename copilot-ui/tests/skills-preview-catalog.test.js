...
function invokeSkillsPreview(engineRoot, elegyHomeAbs) {
...
async function run() {
  ...
  try {
    ...
    writeText(path.join(elegyHomeAbs, 'skills', 'core-guardrails', 'SKILL.md'), '# Core Guardrails\n');
    writeText(path.join(elegyHomeAbs, 'skills-vault', 'core-guardrails', 'SKILL.md'), '# Core Guardrails Vault\n');
    writeText(path.join(elegyHomeAbs, 'skills-vault', 'wolverine-core', 'SKILL.md'), '# Wolverine Core\n');
    writeText(
    ...
    await test('preview returns always-loaded, vault-only, and managed-missing skills', async () => {
      const response = await invokeSkillsPreview(engineRoot, elegyHomeAbs);
    ...
    await test('preview keeps plugin-installed skills distinct with explicit view paths and read-only provenance', async () => {
      writeText(path.join(elegyHomeAbs, 'skills', 'brainstorming', 'SKILL.md'), '# Brainstorming Local\n');
      const response = await invokeSkillsPreview(engineRoot, elegyHomeAbs);
      assert.strictEqual(response.status, 200);

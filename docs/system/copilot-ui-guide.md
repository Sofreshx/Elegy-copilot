...
1. **Desktop shell mode**
   ...
   - For the bounded native install/launch/uninstall smoke lane, run `npm --prefix copilot-ui run desktop:smoke:native`.
   ...
   - The bounded native smoke lane installs that staged NSIS preview into a repo-local scratch path, validates the installed resource layout, verifies the native Tauri window plus loopback health boot, checks first-pass single-instance reuse, and then silently uninstalls.
  - Public GitHub semver tags such as `1.2.3` and `1.2.3-rc.1` remain preview/evaluation releases and stay marked `prerelease=true`; stable desktop selection and downloads come from promoted non-prerelease `desktop-v*` releases.

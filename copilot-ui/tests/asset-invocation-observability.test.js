...
async function run() {
  ...
  try {
    ...
    const projectionStorage = resolveProjectionStorage({ elegyHome });
    const snapshot = {
    ...
    await test('explicit invocation audit records asset.invoked with bounded correlation metadata', async () => {
      ...
      const events = readCatalogAuditEvents(elegyHome, 20);
    ...
    await test('asset audit analytics separate explicit invocation counts from proxy-only fallback usage', async () => {
      appendCatalogAuditEvent(elegyHome, {
      ...
      writeJsonl(path.join(elegyHome, 'session-state', 'session-1', 'events.jsonl'), [
        {
          type: 'session.start',

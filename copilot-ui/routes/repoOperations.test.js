...
async function main() {
  ...
  const routes = register({
    service: {
      ...
      syncRepositories: async (context, input) => {
        ...
        return { contractVersion: 'repo-operations.action.v2', operation: 'sync' };
      },
  ...
  await routes[2].handler({ req: {}, res: startResponse, elegyHomeAbs: 'C:/home/.elegy', engineRoot: 'C:/work/engine' });
  assert.strictEqual(startResponse.status, 201);
  assert.strictEqual(startResponse.body.run.status, 'queued');

  const getResponse = {};
  await routes[3].handler({ req: {}, res: getResponse, match: ['/api/repo-operations/agent-runs/run-1', 'run-1'] });
  assert.strictEqual(getResponse.status, 200);
  assert.strictEqual(getResponse.body.status, 'awaiting-approval');

  const approveResponse = {};
  bodyQueue.push({});
  await routes[4].handler({ req: {}, res: approveResponse, match: ['/api/repo-operations/agent-runs/run-1/approve', 'run-1'] });
  assert.strictEqual(approveResponse.status, 200);
  assert.strictEqual(approveResponse.body.status, 'completed');

  const cancelResponse = {};
  bodyQueue.push({});
  await routes[5].handler({ req: {}, res: cancelResponse, match: ['/api/repo-operations/agent-runs/run-1/cancel', 'run-1'] });
  ...
  assert.strictEqual(calls[2][0], 'agent-start');
  assert.strictEqual(calls[3][0], 'agent-get');
  assert.strictEqual(calls[4][0], 'agent-approve');
  assert.strictEqual(calls[5][0], 'agent-cancel');

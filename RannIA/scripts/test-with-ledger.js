...
// 1. Discover tests
const testDir = path.join(__dirname, '../src/test/suite');
...
// Map .ts to .js for Mocha
const jsTestsToRun = testsToRun.map(f => {
    const relative = path.relative(path.join(__dirname, '../src'), f);
    return path.join(__dirname, '../dist', relative).replace(/\.ts$/, '.js');
});

fs.writeFileSync(includeListFile, JSON.stringify(jsTestsToRun));
...
try {
    ...
    // Run tests
    execSync('node ./dist/test/runTest.js', { 
...
if (fs.existsSync(resultsFile)) {
    ...
    for (const testFile of testsToRun) {
        const relative = path.relative(path.join(__dirname, '../src'), testFile);
        const jsFile = path.join(__dirname, '../dist', relative).replace(/\.ts$/, '.js');
        
        // Mocha uses absolute paths for test.file
        const result = results[jsFile] || { success: false, durationMs: 0, stderr: 'Test did not report results' };
        const hash = testHashes[testFile];

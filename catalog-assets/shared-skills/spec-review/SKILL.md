...
12. **Sibling plan.md:** If the spec has 5+ requirements or 2+ phases, confirm a sibling `plan.md` exists alongside `spec.md`. The spec validator (`validate-specs.js --strict`) produces an advisory warning (not a hard error) when this condition is not met. Use judgment — some specs may intentionally defer the plan to a later phase. See normative spec R8.

...
exports.startWorkflowSidecar = startWorkflowSidecar;
...
const WORKFLOW_SIDECAR_OFF_PATH_MESSAGE = 'Workflow sidecar stays disabled until canonical workflow identifiers and lifecycle binding are aligned.';
...
function buildSidecarUrl(host, port, pathname) {
...
async function probeWorkflowSidecarStatus(statusUrl, bearerToken) {
        ...
        const request = http_1.default.request({
            ...
            response.on('end', () => {
                ...
                if (observedStatus.state !== 'ready'
                    ...
                    reject(Object.assign(new Error(`Workflow sidecar readiness probe did not confirm the canonical sidecar contract: ${reason}`), {
...
async function waitForWorkflowSidecarReady(statusUrl, bearerToken, isCancelled) {
    ...
    throw new Error(lastProbeError
...
async function startWorkflowSidecar(options) {
    ...
    const bearerToken = String(process.env.INSTRUCTION_ENGINE_WORKFLOW_SIDECAR_TOKEN || '').trim()
        || (0, crypto_1.randomBytes)(32).toString('hex');
    const bundledEntry = path_1.default.join(options.runtimeRoot, 'local-tracker', 'dist', 'messagingGateway', 'workflowSidecar.js');
    const baseState = {
    ...
    if (!fs_1.default.existsSync(bundledEntry)) {
        return {
            getPublicState: () => ({
                ...baseState,
                state: 'unavailable',
    ...
    let child = null;
    const localTrackerRoot = path_1.default.join(options.runtimeRoot, 'local-tracker');
    let state = 'unavailable';
    let runtime = baseState.runtime;
    let runtimeBinding = {
    ...
    try {
        child = options.isPackaged
            ? options.shellAdapter?.launchPackagedWorkflowSidecarChild?.({
                localTrackerRoot,
                env,
            }) ?? (0, child_process_1.spawn)(options.processExecPath, (0, gatewayChildMode_1.buildPackagedWorkflowSidecarChildArgs)(), {
                cwd: localTrackerRoot,
                env,
                stdio: 'ignore',
                windowsHide: true,
            })
            : (0, child_process_1.spawn)(process.execPath, [bundledEntry], {
                cwd: localTrackerRoot,
                env,
                stdio: 'ignore',
                windowsHide: true,
    ...
    if (child) {
        ...
        void waitForWorkflowSidecarReady(baseState.healthUrl || '', bearerToken, () => stopped || !child || state === 'error')
            ...
            const observedStatus = isRecord(error?.observedStatus)
                ? {
                    state: normalizeState(error.observedStatus?.state),
                    runtime: normalizeRuntime(error.observedStatus?.runtime),
                    runtimeBinding: normalizeRuntimeBindingState(error.observedStatus?.runtimeBinding),
                }
                : null;
            if (observedStatus) {
                runtime = observedStatus.runtime;

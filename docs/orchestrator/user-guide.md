...
The `@orchestrator` is the single recommended entry point for all complex work in projects using Instruction Engine. It replaces Executive (v1), Executive2, Executive2.5, Executive3, and Executive2-Fast with one unified agent.
...
| `@executive3` | Use `@orchestrator` instead |
| `@executive2-fast` | Use `@orchestrator` instead (trivial requests use fast path) |

The old agents remain functional with deprecation notices but are no longer recommended.

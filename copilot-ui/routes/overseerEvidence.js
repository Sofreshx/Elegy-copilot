...
function register(deps = {}) {
  const client = createLoopbackClient({
    ...
    resolveTarget(pathname) {
      if (pathname === '/api/overseer/evidence/v1/summary') return '/api/evidence/v1/summary';
      return null;
    },
  });
  return [{ method: 'GET', path: '/api/overseer/evidence/v1/summary', handler: client.handler }];
}

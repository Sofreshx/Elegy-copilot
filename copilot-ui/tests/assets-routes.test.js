...
function installPluginAgent(elegyHomeAbs, text) {
  ...
  const linkedAgentAbs = path.join(elegyHomeAbs, 'agents', 'code-reviewer.md');
...
function invokeInstalled(elegyHomeAbs) {
  return new Promise((resolve, reject) => {
    ...
    route.handler({ req: {}, res: {}, elegyHomeAbs });
  });
}
function invokeDelete(elegyHomeAbs, body) {
  return new Promise((resolve, reject) => {
    const routes = register({
...
function invokeView(elegyHomeAbs, relPath, extraDeps = {}) {
  return new Promise((resolve, reject) => {
    const routes = register({
...
function invokeInstallSurfaces(elegyHomeAbs,  body, extraDeps = {}) {
  return new Promise((resolve, reject) => {
    const routes = register({
      readJsonBody: () => Promise.resolve(body),
      sendJson: (_res, status, payload) => resolve({ status, payload }),
...
async function run() {
  ...
  try {
    writeText(
      path.join(elegyHomeAbs, 'skills', 'external-provider', 'brainstorming', 'SKILL.md'),
      [
        '---',
        'name: brainstorming',
    ...
    writeText(
      path.join(elegyHomeAbs, 'skills', 'providers', 'external-provider', 'workflow-kit', 'SKILL.md'),
      '# Workflow Kit\n',
    );
    writeText(
      path.join(elegyHomeAbs, 'skills', 'operations', 'release-drill', 'index.md'),
      '# Release Drill\n',
    );
    writeText(
      path.join(elegyHomeAbs, 'skills-vault', 'on-demand-skill', 'SKILL.md'),
      '# On Demand Skill\n',
    );
    writeText(
      path.join(elegyHomeAbs, 'skills-vault', 'providers', 'external-provider', 'incident-kit', 'index.md'),
      '# Incident Kit\n',
    );
    ...
    writeText(
      path.join(elegyHomeAbs, 'agents', 'providers--external--workflow-guide.md'),
      [
        '---',
        'name: workflow-guide',
    ...
    await test('installed inventory includes plugin-style skills and plain markdown agents', async () => {
      const response = await invokeInstalled(elegyHomeAbs);
    ...
    await test('delete route rejects plugin namespace roots and plain markdown agents', async () => {
      const namespaceDelete = await invokeDelete(elegyHomeAbs, {
      ...
      const pluginAgentDelete = await invokeDelete(elegyHomeAbs, {
    ...
    await test('asset view rejects symlink targets that escape the copilot home root', async () => {
      ...
      const response = await invokeView(elegyHomeAbs, 'agents/code-reviewer.md', {
        fs: {
          existsSync: () => true,
    ...
    await test('asset view accepts Windows-only casing differences for in-root real paths', async () => {
      ...
      const lowerHome = elegyHomeAbs.toLowerCase();
      const response = await invokeView(lowerHome, 'agents/code-reviewer.md', {
        fs: {
          existsSync: () => true,
          realpathSync: () => path.join(elegyHomeAbs.toUpperCase(), 'agents', 'code-reviewer.md'),
        },
        assets: {
          isPointerFile: () => false,
    ...
    await test('install-surfaces forwards target to the backend helper', async () => {
      ...
      assert.strictEqual(Object.prototype.hasOwnProperty.call(receivedOptions, 'elegyHomeAbs'), false);
      assert.strictEqual(Object.prototype.hasOwnProperty.call(receivedOptions, ''), false);
    });

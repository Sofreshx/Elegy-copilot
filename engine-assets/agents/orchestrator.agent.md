...
Model-agnostic compatibility entry point for complex work. This file is intentionally hazards-only. It preserves the non-negotiable routing and execution guardrails, while the canonical workflow, lifecycle, persisted session-state artifact contract, and reviewer split live in:

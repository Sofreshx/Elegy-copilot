...
3. **Two separate sources of truth exist** — the Global tab reads the catalog
   ...
   a separate `GET /api/assets/installed` response from `lib/assets.js`. They can
...
- `scripts/codex-install.mjs`, `scripts/opencode-install.mjs`, `scripts/antigravity-install.mjs`
  — Installer scripts. Use `syncFile`/`syncDirectory`/`syncText` from `install-surface-utils.mjs`.
...
`lib/installSurfaces.js` already returns per-asset results (`created`, `updated`,
...
`catalog.js:1532-1534`) includes an `optedIn: boolean` flag. The UI harness
...
`lib/assets.js` path, line 402) AND reads `catalogState.summary.externalSources`
...
Replace every `expected: true` literal in `routes/catalog.js` with a call to:
...
This lives in `lib/catalogProjectionService.js` (or colocated in `routes/catalog.js`
for locality — spec prefers the projection service for testability).

Known call sites:
- `catalog.js:1173` — `buildManifestInventory`: `expected: true` for each harness
  manifest asset.
- `catalog.js:1276` — `buildExternalHarnessStates` for `cli-tool`: `expected: false`.
...
- **A6.** `node scripts/validate-specs.js specs/asset-sync-truthfulness` passes.
  → verify: exit code 0
- **A7.** The new regression test (`tests/asset-sync-warning-clears.vitest.tsx`)
  passes deterministically on CI.
  → verify: `npx vitest run tests/asset-sync-warning-clears.vitest.tsx` exit code 0
...
- R7: `lib/installLedger.js` created, `isAssetExpectedForUser` helper implemented. `routes/catalog.js:1174` no longer has hard-coded `expected: true` — now reads from the ledger.
- R3: `lib/installLedger.js` — `readInstallLedger`, `writeInstallLedger`, `setHarnessOptIn`, `removeHarnessOptIn` all implemented and exported.
...
- R6: `tests/asset-sync-warning-clears.vitest.tsx` — 3 tests (A, B, A2) all passing. Test A proves warning clears after successful install. Test B proves installer skips are surfaced. Test A2 proves opting out of a harness removes its warnings.
- A6: `node scripts/validate-specs.js specs/asset-sync-truthfulness` — exit code 0.
- A7: `npx vitest run tests/asset-sync-warning-clears.vitest.tsx` — exit code 0, 3 passed.

...
function DetailDrawer({ repo, entity, onClose, onAnalyze, actionsDisabled }: { repo: RepoOperationsRepository | null; entity: EntityRow | null; onClose: () => void; onAnalyze: () => void; actionsDisabled: boolean }) {
  ...
  return <aside aria-label="Repository details" className="repo-operations-detail-drawer">
    ...
    {entity ? <Button disabled={actionsDisabled} onClick={onAnalyze} size="sm" variant="secondary">Analyze blockers</Button> : null}
...
function RepositoryTable({ repositories, onDetail, onPrepare, actionsDisabled }: { repositories: RepoOperationsRepository[]; onDetail: (repo: RepoOperationsRepository) => void; onPrepare: (repo: RepoOperationsRepository) => void; actionsDisabled: boolean }) {
...
export default function RepoOperationsView() {
  ...
  return <div className="view-shell repo-operations-view" data-testid="repo-operations-view"><div className="view-scroll"><PageContainer><header className="repo-operations-command-header" data-testid="repo-operations-toolbar"><div><h1>Repo Operations</h1><p>Safe by default. We analyze before we act.</p></div><div><span>Last scan: {relativeTime(overview.generatedAt)}</span>{state.refreshing ? <span data-testid="repo-operations-refreshing">{state.authorizationFresh ? 'Refreshing repository scan… Current results remain visible.' : 'Refreshing repository scan… Cached results are read-only.'}</span> : null}<Button loading={state.refreshing || state.loading} onClick={() => void repoOperationsStore.loadOverview({ cachedFirst: false })} size="sm" testId="repo-operations-refresh" variant="secondary">Refresh all</Button><Button disabled={!state.authorizationFresh} onClick={() => setPendingAction('fetch')} size="sm" variant="primary">Fetch all remotes</Button></div></header>
    ...
    {overview.warnings.length ? <div className="repo-operations-warning repo-operations-warning-scroll" role="status">{overview.warnings.map((warning) => <p key={warning}>{warning}</p>)}</div> : null}{state.error ? <div className="repo-operations-warning" role="alert"><p>Repository refresh failed: {state.error}</p></div> : null}{state.actionError ? <div className="repo-operations-warning" role="alert"><p>{state.actionError}</p></div> : null}{state.actionResult ? <ActionResultPanel result={state.actionResult} /> : null}
    ...
    <section className="repo-operations-content"><div className="repo-operations-main">{state.activeTab === 'repositories' ? <RepositoryTable actionsDisabled={!state.authorizationFresh} onDetail={(repo) => { setDrawerRepoId(String(repo.repoId)); setDrawerEntityKey(null); }} onPrepare={setPrPickerRepo} repositories={repositories} /> : null}{state.activeTab === 'entities' ? <EntityTable entities={entities} onDetail={(entity) => { setDrawerEntityKey(entityKey(entity)); setDrawerRepoId(entity.repoId); }} onSelect={(entity) => repoOperationsStore.toggleEntitySelection(entityKey(entity))} onSelectAll={(selected) => { const visibleKeys = new Set(entities.map(entityKey)); const retained = state.selectedEntityKeys.filter((key) => !visibleKeys.has(key)); repoOperationsStore.setEntitySelection(selected ? [...retained, ...visibleKeys] : retained); }} selectedKeys={state.selectedEntityKeys} /> : null}{state.activeTab === 'pull-requests' ? <PullRequestTable pullRequests={pullRequests} /> : null}{state.activeTab === 'agent-runs' ? <div className="repo-operations-run-list">{Object.values(state.agentRuns).map((run) => <article className="repo-operations-run-card" key={run.id}><strong>{run.repoLabel} · PR #{run.prNumber}</strong><Badge tone={run.status.includes('awaiting') ? 'accent' : run.status === 'completed' ? 'success' : 'danger'}>{run.status}</Badge><div><Button onClick={() => void repoOperationsStore.refreshAgentRun(run.id)} size="sm" variant="ghost">Refresh run</Button>{run.status.startsWith('awaiting') ? <Button onClick={() => void repoOperationsStore.approveAgentRun(run.id)} size="sm" variant="primary">Approve next step</Button> : null}</div></article>)}{Object.keys(state.agentRuns).length === 0 ? <p className="repo-operations-muted">No agent runs yet.</p> : null}</div> : null}</div><DetailDrawer actionsDisabled={!state.authorizationFresh} entity={drawerEntity} onAnalyze={() => { if (!drawerEntity) return; const key = entityKey(drawerEntity); if (!state.selectedEntityKeys.includes(key)) repoOperationsStore.setEntitySelection([...state.selectedEntityKeys, key]); setPendingAction('analyze'); }} onClose={() => { setDrawerRepoId(null); setDrawerEntityKey(null); }} repo={drawerRepo} /></section>
    {selectedEntities.length ? <div className="repo-operations-batch-bar"><strong>{selectedEntities.length} selected</strong><Button disabled={!state.authorizationFresh} onClick={() => setPendingAction('analyze')} size="sm" variant="secondary">Analyze selected</Button><Button disabled={!state.authorizationFresh} onClick={() => setPendingAction('sync')} size="sm" variant="secondary">Fast-forward eligible</Button><Button disabled={!state.authorizationFresh} onClick={() => setPendingAction('strict-cleanup')} size="sm" variant="secondary">Clean safe</Button><Button disabled={!state.authorizationFresh} onClick={() => setPendingAction('analyzed-cleanup')} size="sm" variant="ghost">Delete analyzed</Button><Button onClick={() => repoOperationsStore.clearEntitySelection()} size="sm" variant="ghost">Clear</Button></div> : null}

...
- `instruction-engine/.github/skills/` folder (global agents).
...
2. **Backup**:
   - Create a timestamped backup of the current `.github/skills/` and `.github/contexts/` folders (e.g., `.backup/pre-upgrade-YYYYMMDD/`).

3. **Migration Check (Folder Structure)**:
   - Check if the target repo is using the old structure (agents in root `agents/`).
   - If so, **Move** them to `.github/skills/` and `.github/contexts/` BEFORE merging.
   - Update `INSTALLATION_GUIDE.md` and `architecture.md` references if they point to the old locations.

4. **Agent Merge Strategy**:
   - Iterate through each skill folder in `.upgrade/skills/`.
   - **If New**: Copy directly to `.github/skills/`.
   - **If Exists**:
...
## Output
- Upgraded `.github/skills/` and `.github/contexts/`.

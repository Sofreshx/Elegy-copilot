...
sequencing, and explicit `RM-*` slice state. It is not an active task list; the current OpenCode
session should plan and implement one coherent roadmap slice.
...
3. Use OpenCode `Plan` mode for that slice only.

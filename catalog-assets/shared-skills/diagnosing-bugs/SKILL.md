...
description: "Diagnosis loop for hard bugs and performance regressions. Use when the user says 'diagnose', 'debug this', or reports something broken, throwing, failing, or slow. Triggers on: diagnose, debug, bug, broken, failing, slow, performance regression, find the cause."
license: Apache-2.0
metadata: {"source":"https://github.com/mattpocock/skills","adapted":true,"originalName":"diagnosing-bugs","notes":"CONTEXT.md→docs/system/, hitl-loop template removed, skill refs updated"}
---
...
10. **HITL bash script.** Last resort. If a human must click, create a structured loop script that drives them through reproduction steps. Captured output feeds back to you.
...
- What is the biggest thing I may still be missing?
- What am I least confident about right now?

If the prevention answer involves architectural change (no good test seam, tangled callers, hidden coupling), hand off to the `improve-codebase-architecture` skill with the specifics. Make the recommendation after the fix is in, not before — you have more information now than when you started.
...
This skill diagnoses bugs. It does not design architecture or perform refactoring — hand off to `improve-codebase-architecture` or `codebase-design` for those. It is model-invoked so it activates automatically when bug/diagnosis language appears.

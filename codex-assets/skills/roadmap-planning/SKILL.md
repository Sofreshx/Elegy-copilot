...
description: "Persisted repository roadmap workflow for Codex. A Roadmap is the durable multi-session planning artifact above session plans: a folder under docs/planning with an index, section files, progress/evidence, and a reevaluation log. Use when raw mixed instructions need ordering into a coherent roadmap, or when taking the next slice from a roadmap for concrete implementation."
---
...
Use a roadmap when work must survive multiple Codex sessions. A **Roadmap** is the durable
repo-visible planning artifact above session plans: it captures goals, non-goals, main targets,
sequencing, section-level progress, evidence, and reevaluation notes. It is not an active task list
and not a session plan; the current Codex session plans and implements one coherent roadmap slice.
...
5. Assign each executable slice a stable id: `RM-<roadmap-slug>-001`, `RM-<roadmap-slug>-002`, etc.
...
4. Make a normal Codex plan for that slice only.
...
- If an agent loses track, stop broad execution and re-anchor on `index.md`, the selected section file, and the selected `RM-*` slice.

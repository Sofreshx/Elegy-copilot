...
repo-visible planning artifact above session plans: it captures goals, main targets,
sequencing, and explicit `RM-*` slice state. It is not an active task list and not a session plan;
the current Codex session plans and implements one coherent roadmap slice.
...
3. Make a normal Codex plan for that slice only.

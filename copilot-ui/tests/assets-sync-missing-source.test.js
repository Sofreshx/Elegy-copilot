...
function invokeSyncAllRoute(engineRoot, elegyHomeAbs, body) {
...
async function run() {
  ...
  try {
    ...
    await test('sync-all skips unreadable sources instead of failing the whole batch', async () => {
      const response = await invokeSyncAllRoute(engineRoot, elegyHomeAbs, {
        dryRun: false,
        force: false,
        pointerMode: true,
      });
      ...
      const installedSkill = path.join(elegyHomeAbs, 'skills', 'valid-skill', 'SKILL.md');
      const vaultedSkill = path.join(elegyHomeAbs, 'skills-vault', 'valid-skill', 'SKILL.md');
      ...
      const auditEvents = readCatalogAuditEvents(elegyHomeAbs, 10);
      assert.ok(
    ...
    await test('sync-all prunes missing-source manifest assets from disk and install state', async () => {
      ...
      assert.ok(!fs.existsSync(path.join(elegyHomeAbs, 'skills', 'missing-skill')), 'Expected missing-source skill install to be pruned');
      assert.ok(!fs.existsSync(path.join(elegyHomeAbs, 'skills-vault', 'missing-skill')), 'Expected missing-source skill vault entry to be pruned');
    ...
    await test('sync-all prunes stale managed assets recorded in the prior install state', async () => {
      ...
      writeText(path.join(elegyHomeAbs, 'skills', 'stale-skill', 'SKILL.md'), '# Stale Skill\n');
      writeText(path.join(elegyHomeAbs, 'skills-vault', 'stale-skill', 'SKILL.md'), '# Stale Skill Vault\n');
      writeText(path.join(elegyHomeAbs, 'agents', 'stale.agent.md'), '# Stale Agent\n');
      writeText(path.join(elegyHomeAbs, 'prompts', 'stale.prompt.md'), '# Stale Prompt\n');
      writeJson(staleStatePath, {
        schemaVersion: 3,
      ...
      assert.ok(!fs.existsSync(path.join(elegyHomeAbs, 'skills', 'stale-skill')), 'Expected stale skill install to be pruned');
      assert.ok(!fs.existsSync(path.join(elegyHomeAbs, 'skills-vault', 'stale-skill')), 'Expected stale skill vault entry to be pruned');
      assert.ok(!fs.existsSync(path.join(elegyHomeAbs, 'agents', 'stale.agent.md')), 'Expected stale agent to be pruned');
      assert.ok(!fs.existsSync(path.join(elegyHomeAbs, 'prompts', 'stale.prompt.md')), 'Expected stale prompt to be pruned');
      const nextState = JSON.parse(fs.readFileSync(staleStatePath, 'utf8'));
      assert.deepStrictEqual(nextState.managedSkills, ['valid-skill']);
      assert.deepStrictEqual(nextState.alwaysLoadedSkills, ['valid-skill']);
      assert.deepStrictEqual(nextState.vaultSkills, ['valid-skill']);

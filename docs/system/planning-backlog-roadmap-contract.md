...
- **Typed Planning Intake** — the canonical typed request layer for audit, roadmap, review-prep, commit-prep,
  CI result checks, and other structured planning requests.
...
- `planning_surface: both` means durable roadmap work happens first, then a plan pack is generated for the active session using the linked durable IDs (`RB-*`, `RM-*`, and `PB-*` when present).
...
| Typed Planning Intake | Capture structured planning requests | What audit, roadmap, review-prep, commit-prep, or CI result check work has been requested? | Freeform bullet capture or accepted backlog state |

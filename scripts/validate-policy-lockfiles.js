...
fail(`missing file: ${toPosixRelative(filePath)}`);
return null;
}
try {
return JSON.parse(fs.readFileSync(filePath, 'utf8'));
} catch (e) {
fail(`failed to parse JSON ${toPosixRelative(filePath)}: ${e.message}`);
return null;
}
}
...
function main() {
if (!fs.existsSync(LOCK_PATH)) fail(`missing file: ${toPosixRelative(LOCK_PATH)}`);

const lock = readJson(LOCK_PATH);
const policy = readJson(POLICY_PATH);
const schema = readJson(SCHEMA_PATH);

if (!lock || !policy || !schema) return;

if (typeof lock !== 'object') fail('lockfile root must be an object');

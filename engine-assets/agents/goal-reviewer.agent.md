...
- `BLOCKED` means the goal list or evidence is insufficient for a reliable assessment. When `BLOCKED`, set `unresolved_goals_path`, `carryover_goals`, and `resolved_goals_to_remove` to `NONE`.
...
- Do not create or edit `docs/issues/unresolved-goals.md`; output only the read-only sync instructions for the downstream docs lane.

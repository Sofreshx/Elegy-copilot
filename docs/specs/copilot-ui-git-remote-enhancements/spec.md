...
- Route modules export `register(context) -> [{ method, path, handler }]`. Registered in `copilot-ui/routes/index.js` via `registry.registerModule(require('./<name>'), context)`. A route's context is the server's merged context (see `copilot-ui/server.js` ~line 5089; includes `elegyHome`, `engineRoot`, `kimakiRuntimeService`, `kimakiCli`).
- Request context `ctx` has: `req`, `res`, `u` (URL — use `ctx.u.searchParams.get(key)` for query params), `pathname`, `elegyHome`, `engineRoot`. See `copilot-ui/routes/_helpers.js`.
- Helpers in `copilot-ui/routes/_helpers.js`: `sendJson(res, code, obj)`, `readJsonBody(req)`, `getQueryParam(ctx, key, fallback)`, `sendError(res, code, msg, codeStr)`.
- `runGit(childProcessImpl, args, cwd, timeoutMs=10000)` and `runCommand(childProcessImpl, command, args, cwd, timeoutMs=10000)` are defined at top of `copilot-ui/routes/git.js` (~lines 13–32). Git subprocess timeout default 10s.
- Config for remote toggle **already exists**: `copilot-ui/lib/copilotConfig.js` `getRemoteSessions(elegyHome)` (default `false`), `setRemoteSessions(elegyHome, bool)`. Routes `GET/PUT /api/config/remote-sessions` exist in `copilot-ui/routes/config.js` (`handleGetRemoteSessions` line 501, `handleSetRemoteSessions` line 520). **The gap:** the desktop runtime ignores this setting and the UI has no toggle.
- Test style: see `copilot-ui/routes/git.test.js` (custom `test()`, `createResponse()`, `createSendJson()`, `findRoute()`, `invoke()`) and `copilot-ui/routes/kimaki.test.js` (`node:test` + `createContext()`). Mirror the style of the file you're editing.
...
`copilot-ui/ui/src/stores/gitStore.ts` `loadRepoState` (line ~96): remove `getGitPullRequest(repoPath)` from the `Promise.all` array and from the destructure/setState. The summary already returns `pullRequest`. **Do not** remove the `getGitPullRequest` export from `copilot-ui/ui/src/lib/api/git.ts` (still used elsewhere). Net: 5 calls → 4.
...
- `copilot-ui/ui/src/views/Workspace/WorkspaceCommitGraph.tsx`: only fetch when expanded. Change the `<details>` in `copilot-ui/ui/src/views/Workspace/WorkspaceGitTab.tsx` (line ~1502) to track open state; pass an `enabled` prop to `WorkspaceCommitGraph` that gates its `useEffect`. Do not fetch on mount when collapsed.
- `copilot-ui/ui/src/views/Workspace/WorkspaceGitTab.tsx`: stagger the mount useEffects — load worktrees first, then on completion kick enriched-worktrees + merge-candidates + stashes. Add simple in-flight booleans to avoid duplicate concurrent calls on re-render.
...
### Phase A tests
- `copilot-ui/routes/git.test.js`: assert that one `loadRepoState` triggers `gh auth status` **at most once** (inject a counting fake `childProcess`). Assert cache hit returns prior result without a new `gh` call. Assert `resolveGitStatus` new parsing matches expected counts.
- `copilot-ui/routes/kimaki.test.js`: add the new route `POST /api/git/pr/refresh` to the route inventory if you snapshot it there (check `copilot-ui/tests/api-contract.test.js`).
...
### Current
`copilot-ui/lib/gitCheckRunner.js` `gateGitAction` (line 422) runs checks for **commit, push, and PR**. `copilot-ui/routes/git.js` `handleGitCommit` (line 422) goes through `handleGitActionWithGate`.

### Target
- **Commit:** no checks, ever. Direct `git commit`.
- **Push:** runs checks. Protected branch (main/master, or remote default head — see `isProtectedBranch` at `copilot-ui/lib/gitCheckRunner.js` line 706) **hard-blocks** on failure, no override. Non-protected branch: `requiresOverride:true` on failure → force-push with override reason.

### C1. Remove gate from commit
`copilot-ui/routes/git.js` `handleGitCommit` (line 422): stop calling `handleGitActionWithGate`. Implement directly:
...
- The push handler (`gitStore.push`, `copilot-ui/ui/src/stores/gitStore.ts` line 236) already sets `checkFailed`/`showOverrideInput` when `response.requiresOverride`. Wire the Force Push UI to read those and call `push()` again with the override reason set via `setUnsafeOverrideReason`.
...
### Current
`copilot-ui/lib/copilotConfig.js` `getRemoteSessions` (line 100, default `false`) and `setRemoteSessions` (line 110) exist. REST `GET/PUT /api/config/remote-sessions` exist (`copilot-ui/routes/config.js`). **Gap:** `copilot-ui/lib/desktop-shell/desktopRuntime/runtimeService.js` line ~227 calls `kimakiRuntimeService.start(...)` **unconditionally**. `RemoteView.tsx` has only Restart.

### E1. Honor the setting at boot
`copilot-ui/lib/desktop-shell/desktopRuntime/runtimeService.js` (~line 227, the `if (kimakiRuntimeService) { kimakiRuntimeService.start(...) }` block):
...
- **Also update the TS source** `copilot-ui/src/desktopRuntime/runtimeService.ts` (the `.js` is compiled from it). Keep both in sync.
...
- Extend `handleStatus` (line 50): include `enabled: copilotConfig.getRemoteSessions(ctx.elegyHome)`. Extend the service with `getPid()`/`getStartedAt()`:
  - `copilot-ui/lib/desktop-shell/desktopRuntime/kimakiRuntimeService.js` (+ `copilot-ui/src/desktopRuntime/kimakiRuntimeService.ts`): store the child process pid and a `startedAt` timestamp (set in `start`, cleared in `stop`/`exit`). Expose `getPid()` and `getStartedAt()`.
...
- Load enabled state: add `enabled` to `RemoteStore` state; fetch from `GET /api/config/remote-sessions` on mount (reuse `copilot-ui/ui/src/lib/api/remote.ts` — add `getRemoteSessionsConfig()`/`setRemoteSessionsEnabled(bool)` if absent).
...
### Phase E tests
- `copilot-ui/lib/desktop-shell/desktopRuntime/runtimeService.test.js`: when `getRemoteSessions` returns `false`, `kimakiRuntimeService.start` is **not** called; when `true`, it is. Inject a fake `copilotConfig` via deps if the runtime service accepts it, else inject via the existing test harness pattern.
- `copilot-ui/routes/kimaki.test.js`: update the registered route list to include the two new routes; add a test that `handleEnable` writes config true and calls `start`, `handleDisable` writes false and calls `stop`, and `handleStatus` includes `enabled`.
...
### Problem
`copilot-ui/routes/git.js` `handleGenerateCommitMessage` (line 1536) runs `opencode run --model X --agent plan --format json` (a tool-using agent), 60s/model, chained across profile-derived models. `parseOpenCodeCommitMessage` (line 1511) uses last-content-wins and can grab reasoning/tool/system text.
...
- `copilot-ui/ui/src/stores/gitStore.ts` `generateCommitMessage` (line 319): on `response.ok === false`, store `response.code` and `response.lastError` on state (add `generateError` / `generateErrorCode` fields to `GitState`).
- `copilot-ui/ui/src/views/Workspace/WorkspaceGitTab.tsx` Generate area (line ~1568): on failure, toast the specific code (`OPENCODE_NOT_FOUND`, `MODEL_CHAIN_FAILED`, `NO_CHANGES`). On `MODEL_CHAIN_FAILED`, add a "Try next model" button that re-invokes `generateCommitMessage` (the backend already iterates models; this is a retry).

### Phase B tests
- `copilot-ui/routes/git.test.js`: feed captured NDJSON fixtures (include tool/reasoning/assistant events) and assert the parser extracts assistant text only. Assert invocation no longer passes `--agent plan` (inject a fake `childProcess` capturing args).
...
### D1. Rich PR resolver
`copilot-ui/routes/git.js` `resolvePullRequest` (line 117): change the `gh pr view --json` fields to include `number,url,state,baseRefName,headRefName,isDraft,statusCheckRollup,reviewDecision,mergeable,mergeStateStatus`. Compute a checks summary (`{ passed, failed, pending }`) from `statusCheckRollup`. Honor the A1 cache. Extend the `GitPullRequestResponse` type in `copilot-ui/ui/src/lib/api/git.ts` and `gitStore` accordingly.

### D2. PR panel in Git tab (feature-branch aware)
`copilot-ui/ui/src/views/Workspace/WorkspaceGitTab.tsx` (replace the small "Collapsible PR create" at line ~1715 with a richer panel):
...
- **Observe:** when a PR exists, show — PR # + state, `base ← head`, checks rollup chips (from summary), `reviewDecision` (APPROVED / REVIEW_REQUIRED / ...), `mergeable` + `mergeStateStatus` (CLEAN / CONFLICT / BLOCKED).
...
### Phase D tests
- `copilot-ui/routes/git.test.js`: PR detail returns the extended shape; merge calls `gh pr merge` with the chosen method; cache is busted after merge.
